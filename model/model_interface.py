...
class MInterface(pl.LightningModule):
    ...
    def on_test_epoch_end(self):
        if hasattr(self, 'scored_rows') and len(self.scored_rows) > 0:
            ...
            print(f"Saved scoring results to test_scored.csv with {len(scored_df)} rows.")

            lambda_threshold = 0.0003
            ...
            print(f"Saved prediction set to prediction_set.csv with {len(pred_df)} samples.")
            ...
            # 排序指标：Hit@k, NDCG@k
            ranking_metrics = evaluate_ranking_metrics(scored_df, ks=[5, 10])
